...
void CAVE::generate() {
    ...
    for (int y = 0; y < 64; ++y) {
        for (int x = 0; x < 64; ++x) {
            double rnoise = (float)((perlin.octave2D_01(((-1) * (x + (r.x * 64)) * 0.5), ((-1) * (y + (r.y * 64)) * 0.5), 1) * 2) - 1 ) * 0.2;
            float levelscale = 0.1f / ((float)level/3);
            const double noise = perlin.octave2D_01(((x + (r.x * 64)) * levelscale), ((y + (r.y * 64)) * levelscale), 2) + rnoise;
...
bool CAVE::isInitialized() const {
    return initalized;
}

bool CAVE::isGenerated() const {
    return initalized;
}
